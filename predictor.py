...
class Predictor:
    ...
    def compute_homography(self, query_path, refer_path, query_is_image=False):
        ...
        return H_m, inliers_num_rate, raw_query_image, raw_refer_image, src_pts_all, dst_pts_all, mask, num_matches, num_inliers

    def align_image_pair(self, query_path, refer_path, show=False):
        H_m, inliers_num_rate, raw_query_image, raw_refer_image, src_pts_all, dst_pts_all, mask, num_matches, num_inliers = self.compute_homography(query_path, refer_path)
        ...
        match_info = {
            ...
            "src_pts": src_pts_all.tolist() if src_pts_all is not None else None,
            "dst_pts": dst_pts_all.tolist() if dst_pts_all is not None else None,
            "mask": mask.tolist() if mask is not None else None,
            "H": H_m.tolist() if H_m is not None else None
        }
